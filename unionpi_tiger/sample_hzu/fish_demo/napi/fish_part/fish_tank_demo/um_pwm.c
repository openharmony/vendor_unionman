...
int init_pmw(int pwmChannel) {
    ...
    (void)memset_s(pwm_file_name, sizeof(pwm_file_name), 0,
                   sizeof(pwm_file_name));

    if (PWM1 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name), "%s/export",
                        PWM1_PEX);
    }
...
int set_pwm_period(int pwmChannel, int period) {
    ...
    if (PWM1 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name), "%s/pwm0/period",
                        PWM1_PEX);
    }
    else if (PWM2 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name), "%s/pwm0/period",
...
int set_pwm_dutyCycle(int pwmChannel, int dutyCycle) {
    ...
    if (PWM1 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name),
                        "%s/pwm0/duty_cycle", PWM1_PEX);
    }
    else if (PWM2 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name),
                        "%s/pwm0/duty_cycle", PWM2_PEX);
    }
    else {
        HILOG_ERROR(LOG_CORE, "PWM WRONOG CHANEEL\n");
        return PWM_WRONOG_CHANNEL;
...
int set_pwm_polarity(int pwmChannel, int polarity) {
    ...
    if (PWM1 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name),
                        "%s/pwm0/polarity", PWM1_PEX);
    }
    else if (PWM2 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name),
                        "%s/pwm0/polarity", PWM2_PEX);
    }
    else {
        HILOG_ERROR(LOG_CORE, "PWM WRONOG CHANEEL\n");
        return PWM_WRONOG_CHANNEL;
...
int set_pwm_enable(int pwmChannel, int isEnable) {
    ...
    if (PWM1 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name), "%s/pwm0/enable",
                        PWM1_PEX);
    }
    else if (PWM2 == pwmChannel) {
        (void)sprintf_s(pwm_file_name, sizeof(pwm_file_name), "%s/pwm0/enable",
    ...
    (void)snprintf_s(buffer, sizeof(buffer), sizeof(buffer), "echo %d > %s",
                     isEnable, pwm_file_name);
    system(buffer);
...
void set_angle(int pwmChannel, int angle, int polarity) {
    printf("setting angle ......\n");
    double x = angle * 1.0 / 180;
    int final_duty = 0;

    // 设置极性
    set_pwm_polarity(PWM1, polarity);
    printf("setting polarity:%d\n", polarity);

    final_duty = (int)((MAX_DUTY - INIT_DUTY) * x) + INIT_DUTY;
    printf("expected dutyCycle:%d  ", final_duty);
    // 设置占空比
    set_pwm_dutyCycle(PWM1, final_duty);
    printf("actually dutyCycle:%d\n", get_pwm_dutyCycle(pwmChannel));

    // 使能pwm
    set_pwm_enable(PWM1, PWM_IS_ENABLED);
}
