...
std::string ShellAbilityProxy::yolo5s(const std::string &dummy)
{
    MessageOption option;
    MessageParcel dataParcel;
    MessageParcel replyParcel;
    dataParcel.WriteString(dummy);
    int error = Remote()->SendRequest(ABILITY_YOLO, dataParcel, replyParcel, option);
    std::string result = (error == ERR_NONE) ? replyParcel.ReadString() : "-1";
    return result;
}

static std::string Yolo5s(const std::string data)
{
    int serverId = 1602L;
    ...
    std::string result = ShellAbility->yolo5s(data.c_str());
...
{
    LedAddOnData *addOnData = (LedAddOnData *)data;
    std::string result = Yolo5s(addOnData->args[0]);
    addOnData->result = strdup(result.c_str()); // 使用 strdup 分配并复制字符串
}
static napi_value Yolo5sWithPromise(napi_env env, napi_callback_info info)
{
    ...
    ZLOGW(LABEL, "Yolo5sWithPromise got");
    // 创建promise
    ...
    ZLOGW(LABEL, "Yolo5sWithPromise working");
...
static napi_value registerdeviceinfoApis(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        DECLARE_NAPI_FUNCTION("Yolo5sWithPromise", Yolo5sWithPromise),
    };
