...
static int LaunchUpgrade(const char *url)
{
    ...
    if (g_sUpgradeStatus == UPGRADE_STATUS_DOWNLOADING ||
            g_sUpgradeStatus == UPGRADE_STATUS_CHECKING ||
            g_sUpgradeStatus == UPGRADE_STATUS_UPDATING) {
    ...
    if (!newHwVersion[0] || strcmp(newHwVersion, g_curHwVersion) ||
            !newSwVersion[0] || !strcmp(newSwVersion, g_curSwVersion)) {
...
static int GetHostIpByUdpConnect(int sock, struct sockaddr_storage *peerAddr, char ip[256])
{
    ...
    connect(sock, (struct sockaddr*)&addrUnspec, sizeof(addrUnspec)); // 不能断开连接
...
static char* BuildDevInfoStr(char *hostIp, uint8_t hostMac[6])
{
    ...
    float temperature = (atoi(thermalZone0Temp) > atoi(thermalZone1Temp)
                             ? atoi(thermalZone0Temp)
                             : atoi(thermalZone1Temp)) /
                        1000.0;
    ...
    (void)snprintf_s(devInfoStr, sizeof(devInfoStr), _TRUNCATE,
             "{\"ip\":\"%s\", \"mac\":\"%02X:%02X:%02X:%02X:%02X:%02X\", \"sn\":\"%s\", "
             "\"hw_version\":\"%s\", \"sw_version\":\"%s\", \"device_model\":\"%s\", "
             "\"static_ip\":\"%d\", \"upgrade_status\":\"%d\", \"uptime\":\"%s\", "
             "\"temperature\":\"%.2f°C\", \"ipcameraPss\":\"%u (kB)\"}",
             hostIp, hostMac[0], hostMac[1], hostMac[2L], hostMac[3L], hostMac[4L], hostMac[5L],
             g_serialNo, g_curHwVersion, g_curSwVersion, g_deviceModel, IsStaticIp(),
             g_sUpgradeStatus, uptime, temperature, ipcameraPss);
