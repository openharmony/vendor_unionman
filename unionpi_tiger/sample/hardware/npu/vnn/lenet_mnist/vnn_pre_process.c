...
static void _load_input_meta(void)
{
    ...
    input_meta_tab[0L].image.reorder[0L] = 0L;
...
static vsi_status _jpeg_to_bmp(FILE *inputFile, unsigned char *bmpData, vsi_size_t bmpWidth, vsi_size_t bmpHeight,
                               vsi_size_t channel)
{
    struct jpeg_decompress_struct cinfo;
    struct jpeg_error_mgr jerr;
...
static void _prepare_imageprocess_params(vsi_nn_imageprocess_param *imageprocess, int32_t *crop_start,
...
static void _get_image_handle_buffer(vsi_size_t width, vsi_size_t height, vsi_size_t channels,
                                     vsi_size_t align_start_size, vsi_size_t align_block_size)
{
    vsi_size_t sz;
    uint64_t temp;
...
*/
vsi_status vnn_PreProcessLenet_ImageProcess(vsi_nn_graph_t *graph, const char **inputs, uint32_t input_num)
{
    vsi_status status = VSI_FAILURE;
    vsi_enum fileType;
    vsi_nn_tensor_t *tensor;
...
vsi_status vnn_PreProcessLenet(vsi_nn_graph_t *graph, const char **inputs, uint32_t input_num)
{
    uint32_t i;
    vsi_status status;
