...
ArrayBuffer aesEcbEncrypt(const ArrayBuffer& key, const ArrayBuffer& plaintext)
{
    ...
    EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), nullptr, key.getData(), iv);
    EVP_EncryptUpdate(ctx, ciphertext.getData(), &len, plaintext.getData(), plaintext.getLength());
    EVP_EncryptFinal_ex(ctx, ciphertext.getData() + len, &len);
