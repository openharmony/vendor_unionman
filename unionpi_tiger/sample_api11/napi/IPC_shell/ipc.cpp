...
public:
  ...
  virtual std::string
  executeCommand(const std::string &dummy) = 0; // 定义业务函数
};
...
std::string ShellAbilityProxy::executeCommand(const std::string &dummy) {
  ...
  int error =
      Remote()->SendRequest(ABILITY_SHELL, dataParcel, replyParcel, option);
...
static napi_value EexecuteCommandNapiWithPromise(napi_env env,
...
static napi_value registerdeviceinfoApis(napi_env env, napi_value exports) {
  napi_property_descriptor desc[] = {
      ...
      DECLARE_NAPI_FUNCTION("EexecuteCommandNapiWithPromise",
                            EexecuteCommandNapiWithPromise),
  };
