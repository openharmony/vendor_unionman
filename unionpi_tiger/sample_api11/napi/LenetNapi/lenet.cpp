...
#define ZLOGE(LOG_LABEL, fmt, args...)                                         \
  (void)OHOS::HiviewDFX::HiLog::Error(LOG_LABEL, "%{public}d: " fmt, __LINE__, \
                                      ##args)
#define ZLOGW(LOG_LABEL, fmt, args...)                                         \
  (void)OHOS::HiviewDFX::HiLog::Warn(LOG_LABEL, "%{public}d: " fmt, __LINE__,  \
                                     ##args)
#define ZLOGI(LOG_LABEL, fmt, args...)                                         \
  (void)OHOS::HiviewDFX::HiLog::Info(LOG_LABEL, "%{public}d: " fmt, __LINE__,  \
                                     ##args)
#define ZLOGD(LOG_LABEL, fmt, args...)                                         \
  (void)OHOS::HiviewDFX::HiLog::Debug(LOG_LABEL, "%{public}d: " fmt, __LINE__, \
                                      ##args)
...
private:
  static inline BrokerDelegator<ShellAbilityProxy>
      delegator_; // 方便后续使用iface_cast宏
};

ShellAbilityProxy::ShellAbilityProxy(const sptr<IRemoteObject> &impl)
    : IRemoteProxy<IShellAbility>(impl) {}

std::string ShellAbilityProxy::lenet(const std::string &dummy) {
  MessageOption option;
  MessageParcel dataParcel;
  MessageParcel replyParcel;
  dataParcel.WriteString(dummy);
  int error =
      Remote()->SendRequest(ABILITY_LENET, dataParcel, replyParcel, option);
  std::string result = (error == ERR_NONE) ? replyParcel.ReadString() : "-1";

  return result;
}

static std::string lenet(const std::string data) {
  int serverId = 1602;
  auto samgr =
  ...
  sptr<IShellAbility> ShellAbility =
      iface_cast<IShellAbility>(remoteObject); // 使用iface_cast宏转换成具体类型

  std::string result = ShellAbility->lenet(data.c_str());
...
static void completeCBForPromise(napi_env env, napi_status status, void *data) {
  ...
  napi_create_string_utf8(env, addOnData->result, strlen(addOnData->result),
                          &result);
...
static void ShellExecuteCB(napi_env env, void *data) {
  LedAddOnData *addOnData = (LedAddOnData *)data;
  std::string result = lenet(addOnData->args[0]);
  addOnData->result = strdup(result.c_str()); // 使用 strdup 分配并复制字符串
}
static napi_value lenetWithPromise(napi_env env, napi_callback_info info) {
  ...
  ZLOGW(LABEL, "lenetWithPromise got");
  // 创建promise
  ...
  napi_status status1;
  size_t strSize1 = 0;
  status1 = napi_get_value_string_utf8(env, args[0], strValue, strSize + 1,
                                       &strSize1);
  ...
  napi_create_string_utf8(env, "ShellExecuteCB", NAPI_AUTO_LENGTH,
                          &resourceName);
  ZLOGW(LABEL, "lenetWithPromise working");
  napi_create_async_work(env, nullptr, resourceName, ShellExecuteCB,
...
static napi_value registerdeviceinfoApis(napi_env env, napi_value exports) {
  napi_property_descriptor desc[] = {
      DECLARE_NAPI_FUNCTION("lenetWithPromise", lenetWithPromise),
  };
