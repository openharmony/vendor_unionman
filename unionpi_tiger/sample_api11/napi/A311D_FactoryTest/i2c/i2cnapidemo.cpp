...
static void i2cNineAxleExecuteCB(napi_env env, void *data) {
  ...
  (i2cHandle == NULL) { I2cClose(i2cHandle); }
...
static napi_value UM_i2c_40Pin_test(napi_env env, napi_callback_info info) {
  ...
  napi_create_async_work(env, nullptr, resourceName, i2c40PinExecuteCB,
...
static napi_value UM_i2c_nine_axles_test(napi_env env,
  ...
  // work，创建成功后通过最后一个参数(addonData->asyncWork)返回asyncwork的handle
  napi_value resourceName = nullptr;
  napi_create_string_utf8(env, "UM_i2c_nine_axles", NAPI_AUTO_LENGTH,
                          &resourceName);
  napi_create_async_work(env, nullptr, resourceName, i2cNineAxleExecuteCB,
                         completeCBForPromise, (void *)addonData,
                         &addonData->asyncWork);
...
static napi_value UM_i2c_rtc_test(napi_env env, napi_callback_info info) {
  ...
  // work，创建成功后通过最后一个参数(addonData->asyncWork)返回asyncwork的handle
  napi_value resourceName = nullptr;
  napi_create_string_utf8(env, "UM_i2c_rtc_test", NAPI_AUTO_LENGTH,
                          &resourceName);
  napi_create_async_work(env, nullptr, resourceName, i2cRtcExecuteCB,
                         completeCBForPromise, (void *)addonData,
                         &addonData->asyncWork);
