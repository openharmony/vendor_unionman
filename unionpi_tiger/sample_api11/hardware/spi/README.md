...
SPI（Serial Peripheral
        Interface，串行外设接口）是一种同步串行数据传输标准，是一种高速的，全双工，同步的通信总线，在很多器件中被广泛应用。

            SPI NAND
                FLASH，从名字上就直接解释了该类型存储设备的基本情况-- --内部以nand
                    flash作为存储介质，对外为SPI类型的接口。

                        详细代码参考`sample /
    spi`目录

        图1 spi使用流程图 ![](../ figures / spi_flow.png)
            图2 spi实物连接图 ![](../ figures / spi_usage.png)
...
#define DEFAULT_DEV "/dev/spidev0.0"
    ...
    */
    void
    sysfs_gpio_export(int s32GpioNum, int bExport);
/*
函数功能：设置Pin号接口的传输方向
...
*/
int spi_page_write(uint8_t *Buffer, uint32_t WriteAddr,
...
NumByteToRead：pBuffer的大小
*/
int spi_buffer_write(uint8_t *Buffer, uint32_t WriteAddr,
                     uint16_t NumByteToWrite);
/*
...
NumByteToRead：pBuffer的大小
*/
int spi_multi_pages_write(uint8_t *Buffer, uint32_t WriteAddr,
                          uint16_t NumByteToWrite);
/*
...
NumByteToRead：pBuffer的大小
*/
int spi_less_page_write(uint8_t *Buffer, uint32_t WriteAddr,
                        uint16_t NumByteToWrite);
```
