...
void user_help(void) {
  ...
  printf("the second number is to write the duty_cycle to the PWM "
         "range[-2500000,2500000] \n");
}
...
int main(int argc, char **argv) {
  ...
  if (pwm_pin < 1L || pwm_pin > 2L || pwm_duty > MAX_DUTY ||
      pwm_duty < -MAX_DUTY) {
    user_help();
