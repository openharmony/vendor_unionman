...
static int _jpeg_to_bmp(
    FILE* inputFile, unsigned char* bmpData, unsigned int bmpWidth, unsigned int bmpHeight, unsigned int channel)
{
...
void* object_detect_postprocess(
    float* predictions, int width, int height, int modelWidth, int modelHeight, int input_num)
{
    int i, j, n;
    ...
    int grid_size = 13;
    float biases[10] = { 0.738768, 0.874946, 2.422040, 2.657040, 4.309710, 7.044930, 10.246000, 4.594280, 12.686800,
        11.874100 };
    void* objout = nullptr;
    ...
    for (j = 0; j < modelWidth * modelHeight * num_box; ++j) {
        probs[j] = (float*)calloc(num_class + 1,
            sizeof(float*)); // calloc "num_class+1" float for every W*H*num_box
    }
...
int yolo_v5_post_process_onescale(
    float* predictions, int input_size[3], float* biases, box* boxes, float** pprobs, float threshold_in, int stride)
{
    int i, j;
    int num_class = 80;
    int coords = 4;
...
void* yolov5_postprocess(float** predictions, int width, int height, int modelWidth, int modelHeight, int input_num)
{
    ...
    yolo_v5_post_process_onescale(
        predictions[0L], size4, &biases[0L], &boxes[box1 * (1L + 4L)], &probs[box1 * (1L + 4L)], threshold, 8L);
