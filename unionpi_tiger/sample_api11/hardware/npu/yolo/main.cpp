...
static const char* coco_names[] = { "person", "bicycle", "car", "motorbike", "aeroplane", "bus", "train", "truck",
...
int run_network(void* qcontext, unsigned char* qrawdata, int fbmode, unsigned char* fbbuf)
{
    ...
    if (use_dma == 1) {
        if (qrawdata != inbuf) {
            ...
            aml_util_flushTensorHandle(qcontext,
                AML_INPUT_TENSOR); // !!note,flush the input buffer in dma mode
        }
    } else {
