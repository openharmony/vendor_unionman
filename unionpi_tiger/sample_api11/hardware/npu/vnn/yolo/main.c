...
{
    return vnn_PostProcessYolov5s(graph);
}
...
static vsi_status vnn_PreProcessNeuralNetwork(vsi_nn_graph_t* graph, int argc, char** argv)
{
    ...
    return vnn_PreProcessYolov5s(graph, inputs, input_num);
}
...
static vsi_nn_graph_t* vnn_CreateNeuralNetwork(const char* data_file_name)
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateYolov5s(data_file_name, NULL, vnn_GetPreProcessMap(), vnn_GetPreProcessMapCount(),
