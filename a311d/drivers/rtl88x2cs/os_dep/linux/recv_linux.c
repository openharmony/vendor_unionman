...
struct rtw_EapolData {
    bool regFlag;  /* is already registered */
    unsigned short count; /* eapol frame count in NetBuffQueue. */
    unsigned short maxCount;
    signed long long enqueueTime;                       /* record eapol frame time for dfx. */
    void (*notify)(const char *name, void *context); /* notify eapol frame enqueue message */
    void *context;
    NetBufQueue eapolQueue;
};

static int rtw_os_indicate_eapol(_adapter *padapter, _pkt *pkt)
{
	u32 offset = 0;
	char* name = ADPT_ARG(padapter);
	struct rtw_EapolData * eapol = (struct rtw_EapolData *)get_rtl_netdev()->specialProcPriv;

	if(pkt ==NULL)
	{
		RTW_INFO("rtw_os_indicate_eapol: alloc eapol pkt packet fail \n");
		goto err_handle;
	}

	NetBufQueueEnqueue(&(eapol->eapolQueue), pkt);
	HdfWifiEventEapolRecv(name, eapol->context);
	return 0;
