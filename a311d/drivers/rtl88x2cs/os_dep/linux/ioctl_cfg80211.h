...
extern struct NetDevice* get_rtl_netdev(void);
...
#ifdef CONFIG_RTL8822CS_WIFI_HDF
#define rtw_cfg80211_rx_mgmt(wdev, freq, sig_dbm, buf, len, gfp) HdfWifiEventRxMgmt(get_rtl_netdev(), freq, 0, buf, len)
#else
...
#else
#ifdef CONFIG_RTL8822CS_WIFI_HDF
#define rtw_cfg80211_mgmt_tx_status(wdev, cookie, buf, len, ack, gfp) HdfWifiEventMgmtTxStatus(get_rtl_netdev(), buf, len, ack)
#else
...
#ifdef CONFIG_RTL8822CS_WIFI_HDF
int cfg80211_rtw_disconnect(struct wiphy *wiphy, struct net_device *ndev, u16 reason_code);
