...
inline int _rtw_netif_rx(_nic_hdl ndev, struct sk_buff *skb)
{
	...
	skb->dev = ndev;
#ifdef CONFIG_RTL8822CS_WIFI_HDF
	return NetIfRxNi(get_rtl_netdev(), skb);
#else
	return netif_rx(skb);
#endif
...
extern void* get_rtl_priv_data(void);
#endif
...
#ifdef CONFIG_RTL8822CS_WIFI_HDF
	struct NetDevice* tempDev = get_rtl_netdev();

    pnetdev = GetLinuxInfByNetDevice(tempDev);
    if (pnetdev == NULL) {
        goto RETURN;
    }

	tempDev->mlPriv = kzalloc(sizeof(struct rtw_netdev_priv_indicator), GFP_KERNEL);
	pnpi = tempDev->mlPriv;
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35))
