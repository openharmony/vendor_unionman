...
#include "OV5647_seq.h"
#include "OV5647_config.h"
...
static sensor_mode_t supported_modes[] = {
    ...
    */
...

    {
        .wdr_mode = WDR_MODE_LINEAR,
        .fps = 30 * 256,
        .resolution.width = 1296,
        .resolution.height = 730,
        .bits = 10,
        .exposures = 1,
        .lanes = 2,
        .bps = 672,
...
static const char p_sensor_data[] = SENSOR__OV5647_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR__OV5647_ISP_SEQUENCE_DEFAULT;
#else
...
//ir_cut_GPIOZ_11, 0: open ir cut, 1: colse ir cut, 2: no operation
   ...
   if (sensor_bp->ir_gname[0] <= 0) {
	...
	pr_info("ir_cut_state = %d\n", ir_cut_state);
	acamera_sbus_write_u8( p_sbus, 0x3A00, ir_cut_state == 1 ? 0x78 : 0x7c );
    return 0;
}
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
	...
	pr_info("[%s.%d] mode:%d,setting_num:%d\n",
		__FUNCTION__,__LINE__,sen_mode, setting_num);
	...
	pr_info("exposures:%d,fps:%d\n",
		param->modes_table[mode].exposures,
		param->modes_table[mode].fps);
	...
	pr_info("s_fps:%d,vmax:%d,max_S:%d,max_L:%d\n",
		p_ctx->s_fps, p_ctx->vmax,p_ctx->max_S, p_ctx->max_L);
	
    param->total.width = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x380C ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x380D );
    param->total.height = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x380E ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x380F );
//	p_ctx->pixel_clock = param->total.width * 
//		param->total.height * p_ctx->s_fps;
	
    param->active.width = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x3808 ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x3809 );
    param->lines_per_second = p_ctx->pixel_clock / param->total.width;
    param->active.height = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x380a ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x380b );
	param->xoffset = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x3810 ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x3811 );
	param->yoffset = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x3812 ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x3813 );	

    param->pixels_per_line = param->total.width;
    param->integration_time_min = SENSOR_MIN_INTEGRATION_TIME;
	pr_info("width:%d, %d,height:%d,%d, lines_per_second:%d,"
		"pixels_per_line:%d,integration_time_min:%d,xoffset:%d,yoffset:%d\n",
		param->total.width, param->active.width,
		param->total.height,param->active.height,
		param->lines_per_second,param->pixels_per_line,
		param->integration_time_min,param->xoffset,
		param->yoffset);
...
void sensor_deinit_ov5647( void *ctx )
{
    sensor_context_t *t_ctx = ctx;
...
//--------------------Initialization------------------------------------------------------------
void sensor_init_ov5647( void **ctx, sensor_control_t *ctrl, void *sbp )
{
  ...
  //  s_ctx.dgain_limit = 15 << DGAIN_PRECISION;
   ...
   // s_ctx.param.dgain_log2_max = 4 << LOG2_GAIN_SHIFT;
	//HTS * VTS * FPS
    s_ctx.pixel_clock = 80000000;//81612480 ;//108183240;
...
//    s_ctx.param.dgain_log2_max = 0;
    ...
    s_ctx.param.mode = 0;
	s_ctx.param.xoffset = 0;
    s_ctx.param.sensor_ctx = &s_ctx;
    s_ctx.param.isp_context_seq.sequence = p_isp_data;
    s_ctx.param.isp_context_seq.seq_num = SENSOR_OV5647_ISP_CONTEXT_SEQ;
...
int sensor_detect_ov5647( void* sbp)
{
    static sensor_context_t s_ctx;
    int ret = 0;
    s_ctx.sbp = sbp;
    ...
    if (sensor_get_id(&s_ctx) == 0xFFFF)
        ...
        pr_info("sensor_detect_ov5647:%d\n", ret);
