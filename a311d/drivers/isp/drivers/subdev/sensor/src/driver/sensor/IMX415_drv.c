...
#define zgs 0
#define WDR_2_DOL_4K_BRL 2228
#define WDR_2_DOL_2K_BRL 1115
...
static void sensor_alloc_integration_time( void *ctx, uint16_t *int_time_S, uint16_t *int_time_M, uint16_t *int_time_L )
{
    ...
    case WDR_MODE_LINEAR: // Normal mode
        if ( *int_time_S > p_ctx->vmax - 4 ) *int_time_S = p_ctx->vmax - 4;
        if ( *int_time_S < 1 ) *int_time_S = 1;
        tmp = p_ctx->vmax - *int_time_S - 4;
    ...
    case WDR_MODE_FS_LIN: // DOL3 Frames
        ...
        if ( p_ctx->int_time_S != *int_time_S || p_ctx->int_time_M != *int_time_M || p_ctx->int_time_L != *int_time_L ) {
            ...
            p_ctx->shs3 = p_ctx->frame - *int_time_L - 4;
            p_ctx->shs1 = p_ctx->rhs1 - *int_time_M - 4;
            p_ctx->shs2 = p_ctx->rhs2 - *int_time_S - 4;
        }
        ...
        if ( p_ctx->int_time_S != *int_time_S || p_ctx->int_time_L != *int_time_L ) {
            ...
            p_ctx->shs2 = p_ctx->frame - *int_time_L - 4;
            p_ctx->shs1 = p_ctx->rhs1 - *int_time_S - 4;
        }
#endif
        break;
    }
...
static void sensor_update( void *ctx )
{
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt ) {
        ...
        if ( p_ctx->gain_cnt ) {
            p_ctx->gain_cnt--;
            acamera_sbus_write_u8( p_sbus, 0x3090, ( uint8_t )( p_ctx->again[p_ctx->again_delay] ));
        }
        ...
        if ( p_ctx->int_cnt ) {
            ...
            case WDR_MODE_LINEAR:
                acamera_sbus_write_u8( p_sbus, 0x3051, ( p_ctx->int_time_S >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3050, ( p_ctx->int_time_S >> 0 ) & 0xFF );
                break;
...
#endif
                // SHS1
                acamera_sbus_write_u8( p_sbus, 0x3055, ( p_ctx->shs1_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3054, ( p_ctx->shs1_old >> 0 ) & 0xFF );

                // SHS2
                acamera_sbus_write_u8( p_sbus, 0x3051, ( p_ctx->shs2_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3050, ( p_ctx->shs2_old >> 0 ) & 0xFF );
                break;
            }
        }
...
static uint16_t sensor_get_id( void *ctx )
{
	...
	LOG(LOG_CRIT, "%s: success to read sensor %x\n", __func__, sensor_id);
    return sensor_id;
}
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
	...
	LOG(LOG_CRIT, "mode = %d, setting_num = %d\n", mode, setting_num);
	...
	LOG(LOG_CRIT, "wdr_mode = %d\n", param->modes_table[mode].wdr_mode);
    ...
    case WDR_MODE_FS_LIN:
        ...
        if ( param->modes_table[mode].exposures == 2 ) {
            ...
            sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, SENSOR_IMX415_SEQUENCE_DEFAULT_WDR_1080P );
        }
        p_ctx->s_fps = 30;
        break;
    default:
    ...
    if ( (param->modes_table[mode].exposures == 1) && (param->modes_table[mode].fps == 30 * 256) ) {
        ...
        p_ctx->s_fps = 30;
        p_ctx->vmax = 2243;
    } else if (param->modes_table[mode].fps == 60 * 256) {
        p_ctx->s_fps = 60;
        p_ctx->vmax = 1908;
    } else {
    ...
    switch ( r ) {
    case 0: // full size
        param->active.width = 3840;
        param->active.height = 2160;
...
#if 1
        acamera_sbus_write_u8( p_sbus, 0x3061, ( p_ctx->rhs1 >> 8 ) & 0xFF );
        acamera_sbus_write_u8( p_sbus, 0x3060, ( p_ctx->rhs1 >> 0 ) & 0xFF );
#endif
        //acamera_sbus_write_u8( p_sbus, 0x3035, ( p_ctx->rhs2 >> 8 ) & 0xFF );
        //acamera_sbus_write_u8( p_sbus, 0x3034, ( p_ctx->rhs2 >> 0 ) & 0xFF );
    }

    param->total.width = ( (uint16_t)acamera_sbus_read_u8( p_sbus, 0x3025 ) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x3024 );
...
static void start_streaming( void *ctx )
{
    ...
    // trigger auto calibration process for the imx415 sensor
...
*/
...
#elif PLATFORM_C308X
		write2_reg(0x11400400, 0xfe000428);
#endif
    ...
    s_ctx.param.isp_context_seq.seq_num= SENSOR_IMX415_CONTEXT_SEQ;
