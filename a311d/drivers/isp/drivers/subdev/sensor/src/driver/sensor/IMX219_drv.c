...
#include "IMX219_seq.h"
#include "IMX219_config.h"
...
#define VBLANK_30FPS 0x0990 // 2448 lines
#define VBLANK_15FPS 0x1320 // 4896 lines
...
static const char p_sensor_data[] = SENSOR_IMX219_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR_IMX219_ISP_CONTEXT_SEQUENCE;
#else
...
static void sensor_update( void *ctx )
{
    ...
    if ( p_ctx->change_flg || p_ctx->again_change || p_ctx->dgain_change ) {
        /*fast standby enable*/
        acamera_sbus_write_u8( p_sbus, 0x100, 0x00 );
        if ( p_ctx->again_change ) {
            // ---------- ANA_GAIN_GLOBAL_A -------------
            acamera_sbus_write_u8( p_sbus, 0x157, ( uint8_t )( p_ctx->again[p_ctx->again_delay] ) );
            p_ctx->again_change--;
        }

        if ( p_ctx->dgain_change ) {
            // --------- DIG_GAIN_GLOBAL_A  -------------
            acamera_sbus_write_u8( p_sbus, 0x158, ( uint8_t )( p_ctx->dgain[p_ctx->dgain_delay] >> 8 ) );
            acamera_sbus_write_u8( p_sbus, 0x159, (uint8_t)p_ctx->dgain[p_ctx->dgain_delay] & 0xFF );
            p_ctx->dgain_change--;
        }

        if ( p_ctx->change_flg ) {
            // -------- COARSE_INTEGRATION_TIME ----------
            acamera_sbus_write_u8( p_sbus, 0x15a, ( uint8_t )( p_ctx->int_time >> 8 ) );
            acamera_sbus_write_u8( p_sbus, 0x15b, ( uint8_t )( p_ctx->int_time & 0xFF ) );
            p_ctx->change_flg = 0;
        }

        /*fast standby disable*/
        acamera_sbus_write_u8( p_sbus, 0x100, 0x01 );
    }
...
static uint16_t sensor_get_id( void *ctx )
{
    ...
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x0000) << 8;
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x0001);
    ...
    pr_err("imx219: read sensor id 0x%x\n", sensor_id);

    LOG(LOG_ERR, "%s: read sensor id 0x%x\n", __func__, sensor_id);
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    /* LINE_LENGTH_A: line_length_pck (Units: Pixels) */
    p_ctx->HMAX = ( acamera_sbus_read_u8( p_sbus, 0x162 ) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x163 );
    /* FRM_LENGTH_A frame_length_lines */
    p_ctx->VMAX = ( acamera_sbus_read_u8( p_sbus, 0x160 ) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x161 );

	
	param->xoffset = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x164 ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x165 );
	param->yoffset = ( (uint16_t)(acamera_sbus_read_u8( p_sbus, 0x168 ) & 0x0f) << 8 ) | acamera_sbus_read_u8( p_sbus, 0x169 );	

    p_ctx->int_max = p_ctx->HMAX;//VBLANK_30FPS;
	...
	pr_info("[%s.%d]  active{%d, %d}, total {%d, %d} offset{%d,%d}\n", __FUNCTION__,__LINE__,
		param->active.width, param->active.height,
		param->total.width, param->total.width,
		param->xoffset, param->yoffset);
    ...
    LOG( LOG_CRIT, "Mode %d, Setting num: %d, Total:%dx%d, RES:%dx%d",
                   mode, setting_num,
...
static void sensor_get_lines_per_second( void *ctx )
{
    ...
    uint32_t master_clock = 265600000;
...
void sensor_deinit_imx219( void *ctx )
{
    ...
    pr_err("imx219: sensor_deinit_imx219\n");
}
//--------------------Initialization------------------------------------------------------------
void sensor_init_imx219( void **ctx, sensor_control_t *ctrl, void* sbp)
{
    // Local sensor data structure
    static sensor_context_t s_ctx;

    pr_err("sensor_init_imx219()\n");
    ...
    *ctx = &s_ctx;
    ...
    s_ctx.sbus.bus = 3;//get_next_sensor_bus_address();
    ...
    s_ctx.param.isp_context_seq.seq_num= SENSOR_IMX219_CONTEXT_SEQ;
...
int sensor_detect_imx219( void* sbp)
{
    static sensor_context_t s_ctx;
    int ret = 0;
    //sensor_bringup_t* sensor_bp = (sensor_bringup_t*) sbp;
    ...
    if (sensor_get_id(&s_ctx) == 0xFFFF)
        ...
        pr_info("sensor_detect_imx219:%d\n", ret);
