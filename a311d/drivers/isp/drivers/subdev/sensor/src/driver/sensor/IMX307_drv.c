...
static sensor_mode_t supported_modes[] = {
	{
		.wdr_mode = WDR_MODE_LINEAR, // 2 Lanes
		.fps = 30 * 256,
		...
		.bayer = BAYER_RGGB,
		.dol_type = DOL_NON,
		.num = SENSOR_IMX307_SEQUENCE_1080P_30FPS_10BIT_2LANE,
	},
#if 0
    {
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
	...
	pr_info("[%s.%d] mode :%d, setting_num:%d\n", __FUNCTION__,__LINE__,mode, setting_num);
	...
	pr_info("active:{%d,%d},total:{%d,%d}\n",
		param->active.width, param->active.height,
		param->total.width, param->total.height);
