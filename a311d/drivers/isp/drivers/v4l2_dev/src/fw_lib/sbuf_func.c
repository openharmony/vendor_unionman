...
static int sbuf_mgr_alloc_sbuf( struct sbuf_mgr *p_sbuf_mgr )
{
    ...
    LOG( LOG_CRIT, "sbuf: len_needed: %zu, len_alloc: %u, len_used: %u, page_size: %lu, buf_alloc: %lu, buf_used: %lu.",
         sizeof( struct fw_sbuf ), p_sbuf_mgr->len_allocated, p_sbuf_mgr->len_used, PAGE_SIZE, (unsigned long)p_sbuf_mgr->buf_allocated, (unsigned long)p_sbuf_mgr->buf_used );
...
static void sbuf_mgr_apply_new_param( struct sbuf_context *p_ctx, struct sbuf_idx_set *p_idx_set )
{
    ...
    if ( p_idx_set->awb_idx_valid && ( p_idx_set->awb_idx < SBUF_STATS_ARRAY_SIZE ) ) {
        ...
        //unionman add 20200107 xxx
        if(p_sbuf_awb->awb_red_gain - 60 > 0){
            p_sbuf_awb->awb_red_gain -= 60;
        }
        if(p_sbuf_awb->awb_blue_gain - 60 > 0){
            p_sbuf_awb->awb_blue_gain -= 60;
        }
        //end
