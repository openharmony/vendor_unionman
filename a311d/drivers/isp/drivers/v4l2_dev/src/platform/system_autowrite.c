...
uint32_t autowrite_ds2_ping_start( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_PING_ADDR0_ST );
	}

    return result;
}

uint32_t autowrite_ds2_pong_start( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_PONG_ADDR0_ST );
	}

    return result;
}

uint32_t autowrite_ds2_image_buffer_stride_read( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_FRAME_SIZE0 );
	}

    return result;
}

uint32_t autowrite_ds2_image_buffer_stride_read1( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_FRAME_SIZE1 );
	}

    return result;
}

uint32_t autowrite_ds2_writer_frame_wcount_read( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_COUNT );
	}
	
    return result;
}

uint32_t autowrite_ds2_writer_memsize_read( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_FRAME_BUF_SIZE0 );
	}
	
    return result;
}

uint32_t autowrite_ds2_writer_memsize_read1( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_FRAME_BUF_SIZE1 );
	}
	
    return result;
}

uint32_t autowrite_ds2_drop_jump_read( void )
{
	uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
		result = (ioread32( p_hw_ds2_base + MIPI_BL_CTRL1 ) & 0xFF0000) >> 16;
	}

	return result;
}

uint32_t autowrite_ds2_drop_write_read( void )
{
	uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
		result = (ioread32( p_hw_ds2_base + MIPI_BL_CTRL1 ) & 0xFF00) >> 8;
	}

	return result;
}

uint32_t autowrite_ds2_drop_mode_status( void )
{
	uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
		result = (ioread32( p_hw_ds2_base + MIPI_BL_CTRL0 ) & (1 << 5));
	}
...
uint32_t autowrite_ds2_start_address_read( void )
{
    uint32_t result = 0;

	if ( p_hw_ds2_base != NULL ) {
       result = ioread32( p_hw_ds2_base + MIPI_BL_FRAME_BUFFER_START_ADDR0 );
	}
...
uint32_t autowrite_ds2_stop( void )
{
    uint32_t result = 0;
    uint32_t val = 0;
	...
	if ( p_hw_ds2_base != NULL ) {
		...
		val = val | (0x1 << 4);
	    void *ptr = (void *)( p_hw_ds2_base + MIPI_BL_CTRL0 );
		iowrite32(val, ptr); // ladder stop next frame
...
static void auto_write_ds2_cfg_buff(void *cfg)
{
	auto_write_cfg_t *a_cfg = NULL;
	...
	iowrite32(0x00000000, p_hw_ds2_base + MIPI_BL_FRAME_TH);
	iowrite32(a_cfg->frame_buffer_start0, p_hw_ds2_base + MIPI_BL_FRAME_BUFFER_START_ADDR0P);
	iowrite32(a_cfg->frame_buffer_start0, p_hw_ds2_base + MIPI_BL_FRAME_BUFFER_START_ADDR0);
	iowrite32(a_cfg->frame_size0, p_hw_ds2_base + MIPI_BL_FRAME_SIZE0);
	iowrite32(a_cfg->frame_buffer_size0, p_hw_ds2_base + MIPI_BL_FRAME_BUF_SIZE0);
	iowrite32(a_cfg->frame_buffer_start1, p_hw_ds2_base + MIPI_BL_FRAME_BUFFER_START_ADDR1);
	iowrite32(a_cfg->frame_size1, p_hw_ds2_base + MIPI_BL_FRAME_SIZE1);
	iowrite32(a_cfg->frame_buffer_size1, p_hw_ds2_base + MIPI_BL_FRAME_BUF_SIZE1);
}
...
static void auto_write_ds2_reset(uint32_t p_type)
{
	iowrite32(1 << 15, p_hw_ds2_base + MIPI_BL_CTRL0);
	mdelay(1);
	iowrite32(0 << 15, p_hw_ds2_base + MIPI_BL_CTRL0);
}
...
static void auto_write_ds2_cfg_drop(void *cfg)
{
	auto_write_cfg_t *a_cfg = NULL;
	uint32_t val = 0;
	...
	iowrite32(val, p_hw_ds2_base + MIPI_BL_CTRL1);
}
...
void auto_write_cfg_ds2_ping_pong(void *cfg)
{
	auto_write_cfg_t *a_cfg = NULL;
	...
	iowrite32(a_cfg->ping_st0, p_hw_ds2_base + MIPI_BL_PING_ADDR0_ST);
	iowrite32(a_cfg->ping_ed0, p_hw_ds2_base + MIPI_BL_PING_ADDR0_ED);
	iowrite32(a_cfg->pong_st0, p_hw_ds2_base + MIPI_BL_PONG_ADDR0_ST);
	iowrite32(a_cfg->pong_ed0, p_hw_ds2_base + MIPI_BL_PONG_ADDR0_ED);
	iowrite32(a_cfg->ping_st1, p_hw_ds2_base + MIPI_BL_PING_ADDR1_ST);
	iowrite32(a_cfg->ping_ed1, p_hw_ds2_base + MIPI_BL_PING_ADDR1_ED);
	iowrite32(a_cfg->pong_st1, p_hw_ds2_base + MIPI_BL_PONG_ADDR1_ST);
	iowrite32(a_cfg->pong_ed1, p_hw_ds2_base + MIPI_BL_PONG_ADDR1_ED);
...
void auto_write_ds2_init(void *cfg)
{
	auto_write_cfg_t *a_cfg = NULL;
	...
	auto_write_cfg_ds2_ping_pong(cfg);

	auto_write_ds2_reset(a_cfg->p_type);

	auto_write_ds2_cfg_buff(cfg);

	auto_write_ds2_cfg_drop(cfg);
}
...
void auto_write_ds2_start(void *cfg)
{
	auto_write_cfg_t *a_cfg = NULL;
	uint32_t val = 0;
	...
	val = ioread32(p_hw_ds2_base + MIPI_BL_CTRL0);
	val = val | ((a_cfg->drop_enable & 0x1) << 5);
	iowrite32(val, p_hw_ds2_base + MIPI_BL_CTRL0); // ladder enable drop mode

	val = val | (1 << 13) | (1 << 0);
	iowrite32(val, p_hw_ds2_base + MIPI_BL_CTRL0);// ladder enable
