...
#else
            kfree_sensitive( dev->pstreams[sp->stream_id] );
#endif
            dev->pstreams[sp->stream_id] = NULL;
        }
    }
...
static int isp_v4l2_qbuf( struct file *file, void *priv, struct v4l2_buffer *p )
{
    ...
    rc = vb2_qbuf( &sp->vb2_q, vdev->v4l2_dev->mdev, p );
#endif
			...
			int buf_type, struct v4l2_fract *aspect)
{
    ...
    if (pstream->stream_type == V4L2_STREAM_TYPE_FR ||
        ...
        ret = isp_v4l2_get_crop(pstream, &crop);
        if (!ret) {
            aspect->numerator = crop.c.width;
            aspect->denominator = crop.c.height;
        }
        pr_info("%s: aspect = (%u, %u), ret=%d\n", __func__, aspect->numerator, aspect->denominator, ret);
    } else
        LOG(LOG_ERR, "isp_v4l2_g_pixelaspect Error support this stream type: %d", pstream->stream_type);
...
#else
    .vidioc_enum_fmt_sdr_cap = isp_v4l2_enum_fmt_vid_cap,
#endif
