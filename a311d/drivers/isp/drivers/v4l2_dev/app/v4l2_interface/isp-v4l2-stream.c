...
/* default size & format */
#define ISP_DEFAULT_FORMAT V4L2_PIX_FMT_RGB24 
//V4L2_PIX_FMT_RGB32
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0))
extern struct timespec64 normal_fftt;
#else
extern struct timespec64 normal_fftt;
#endif
...
#else
    if( g_fftt ) {
        ktime_get_real_ts64(&normal_fftt);
        g_fftt = 0;
    }
#endif
}
...
#else
    interval_val += ( ( end.tv_nsec - begin.tv_nsec ) / 1000000 );
#endif
...
static int isp_v4l2_stream_copy_thread( void *data )
{
    ...
    LOG( LOG_CRIT, "[Stream#%d] Thread Exit", pstream->stream_id );
...
/* for now, we need memcpy */
    ...
    if ( fw_intf_stream_start( pstream->ctx_id, pstream->stream_type ) < 0 ) {
        LOG( LOG_ERR, "[Stream#%d] fw_intf_stream_start failed", pstream->stream_id );
        return -1;
    }
...
int isp_v4l2_stream_try_format( isp_v4l2_stream_t *pstream, struct v4l2_format *f )
{
    ...
    if (f->fmt.pix_mp.pixelformat == ISP_V4L2_PIX_FMT_META) {
        ...
        if ( !tfmt ) {
            LOG( LOG_CRIT, "qqqq [Stream#%d] format 0x%08x is not supported, setting default format 0x%08x.\n",
