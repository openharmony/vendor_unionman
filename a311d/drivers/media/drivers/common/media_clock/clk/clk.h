...
static const struct of_device_id media_clk_dt_match[] = {
	{
...
static s32 media_clk_probe(struct platform_device *pdev)
{
	s32 ret;
	ret = amports_clock_gate_init(&pdev->dev);
	pr_info("%s: amports_clock_gate_init() ret=%d\n", __func__, ret);
	return ret;
}

static struct platform_driver media_clk_driver = {
	.probe = media_clk_probe,
	.driver = {
		.name = "amlogic_media_clk",
		.of_match_table = media_clk_dt_match,
	}
};
...
static int __init vdec_init_clk(void)
{
	...
	if (platform_driver_register(&media_clk_driver)) {
		pr_err("failed to register media_clk_driver\n");
		return -ENODEV;
	}
