...
#ifndef CONFIG_AMLOGIC_IONVIDEO

static inline int ionvideo_assign_map(char **receiver_name, int *inst)
{
	pr_err("Error: %s() is not supported.\n", __func__);
	return -1;
}

static inline int ionvideo_alloc_map(int *inst)
{
	pr_err("Error: %s() is not supported.\n", __func__);
	return -1;
}

static inline void ionvideo_release_map(int inst)
{
	pr_err("Error: %s() is not supported.\n", __func__);
}
