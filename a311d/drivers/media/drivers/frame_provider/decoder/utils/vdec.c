...
{
	pr_err("Error: %s() is not supported.\n", __func__);
	return -1;
}
...
static int vdec_core_thread(void *data)
{
	...
	//sched_setscheduler(current, SCHED_FIFO, &param); //FIXME
...
int stream_buffer_status_show(char *buf,
...
EXPORT_SYMBOL(stream_buffer_status_show);

static ssize_t vfm_path_store(struct class *class,
		 struct class_attribute *attr,
		 const char *buf, size_t count)
{
...
static ssize_t vfm_path_show(struct class *class,
	struct class_attribute *attr, char *buf)
{
	int len = 0;
	int i;
