...
{
	struct vdec_avs_hw_s *hw = container_of(timer, struct vdec_avs_hw_s, recycle_timer);
...
static s32 vavs_init(struct vdec_avs_hw_s *hw)
{
	...
	*/
	...
	if (hw->m_ins_flag) {
		timer_setup(&hw->check_timer, NULL, 0);
...
static void check_timer_func(struct timer_list *timer)
{
	struct vdec_avs_hw_s *hw = container_of(timer, struct vdec_avs_hw_s, check_timer);
	struct vdec_s *vdec = hw_to_vdec(hw);
